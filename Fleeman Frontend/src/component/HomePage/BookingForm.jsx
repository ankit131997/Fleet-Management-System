...
export const BookingForm = () => {
  ...
  const fetchCitiesData = async (stateId) => {
    try {
      const response = await fetch(`http://localhost:8080/city/${stateId}`);
      const data = await response.json();
                ...
                <Autocomplete
                  ...
                  onChange={(event, newValue) => {
                    ...
                    if (pickupState && newValue) {
                      fetchHubData(`http://localhost:8080/api/v1/hub?stateName=${pickupState.stateName}&cityName=${newValue}`, setPickupHubData);
                    }
                  }}
            ...
            {searchOption === "airport" && (
              <TextField
                ...
                onChange={(e) => {
                  ...
                  fetchHubData(`http://localhost:8080/api/v1/airport?airportCode=${e.target.value}`, setPickupHubData);
                }}
                sx={{ mt: 2 }}
              />
            )}
